...
| Step                        | What It Does                                                                                                                                                                                                                                                                                                                                                                   | Example (English Inputs)                                                                                                                                                                                         | Example Output Type                                                                                                                                                                                                                                            |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Load Job Context            | Receives `jobDescriptionText`, `jobId`, and `locationId`. In demo mode, it scans the job description for specific keywords (e.g., “forklift”, “warehouse”, “night”) and builds a `requirementsSchema` with basic must-have and nice-to-have indicators. It stores it as json object in the same way for any input (vs. LLM which might create differnt object per input). It also returns rule metadata (`roleId`, `locationId`, `jdVersion`, compliance flags) for auditability and versioning. | **Job Description (EN):** “We are hiring a Warehouse Associate for night shifts. Minimum 2 years of warehouse experience required. Forklift certification required.” → detects `warehouse`, `forklift`, `night`. | Structured job context:<br>`{ "jobId": "warehouse-role", "locationId": "paris-01", "requirementsSchema": { "must_have_keywords": ["forklift", "night"], "nice_to_have_keywords": ["warehouse"], "certifications": ["forklift"] }, "jdVersion": "JD_DEMO_V1" }` |
| Ingest Application Event    | Receives raw payload from UI (`applicationId`, `cvText`, `screeningAnswersText`, `jobDescriptionText`, `jobId`, `locationId`, `availability`). It standardizes structure into a clean `application` object. It does not evaluate or score — only ensures consistent schema for downstream steps.                                                                               | **CV (EN):** “John Doe. 3 years warehouse experience. Certified forklift operator.”<br>**Answers (EN):** “Yes, available for night shifts.”                                                                      | Clean JSON object:<br>`{ "application": { "applicationId": "123", "cvText": "John Doe... 3 years warehouse experience...", "screeningAnswersText": "Available for night shifts.", "jobId": "warehouse-role", "locationId": "paris-01" } }`                     |
| Normalize & De-Dupe         | Combines `cvText` + `screeningAnswersText`, lowercases text, removes extra whitespace, and generates a deterministic SHA-256 fingerprint to prevent duplicate processing at scale. Fully language-agnostic and deterministic.                                                                                                                                                  | `"JOHN DOE\n\n3 YEARS EXPERIENCE"` → `"john doe 3 years experience"`<br>Generates dedupe key: `a3f91c7b2e4d8f10`                                                                                                 | Canonicalized string + hash:<br>`{ "dedupe": { "dedupeKey": "a3f91c7b2e4d8f10", "isDuplicate": false, "normalizedLength": 58 } }`                                                                                                                              |
| Structured Extraction (LLM) | Sends job description + application content to Bedrock / Claude. The model semantically extracts structured fields into a strict JSON schema (`years_experience`, `has_required_certification`, `skills`, `availability`, `confidence`). Temperature ≈ 0 ensures stable structured output. The model also assigns a confidence score based on clarity of extracted evidence.   | From CV + answers:<br>“3 years warehouse experience. Certified forklift operator. Available for night shifts.”                                                                                                   | Structured evaluation fields:<br>`{ "years_experience": 3, "has_required_certification": true, "education_level": "high school", "skills": ["warehouse operations", "forklift"], "availability": "night shifts", "confidence": 86 }`                           |
| Fit Scoring (Deterministic) | Consumes extracted numeric/boolean fields and applies fixed scoring logic: +40 if experience ≥2, +30 if certification true, +20 if availability confirmed, +10 if confidence >70. Computes total score, score band, and final decision via rule-based thresholds. LLM generates short explanation but does not influence outcome.                                              | Extracted: `3 years`, `certification=true`, `availability confirmed`, `confidence=86` → Score = 40+30+20+10 = 100 → Interview Scheduled                                                                          | Final decision object:<br>`{ "score": 100, "scoreBand": "high", "missingInfoList": [], "confidence": 86, "finalDecision": "Interview Scheduled" }`                                                                                                             |
...
| Normalize & De-Dupe         | The normalizaton is required to perfrom deduplication. Canonicalizes text (lowercase + whitespace normalization) and generates a deterministic SHA-256 fingerprint to detect duplicates. Works regardless of language.                                 | `"ИВАН ПЕТРОВ\n\n4 ГОДА ОПЫТА"` → `"иван петров 4 года опыта"`<br>Generates dedupe key: `a3f91c7b2e4d8f10`                                                                     | Clean string + hash:<br>`{ "dedupeKey": "a3f91c7b2e4d8f10", "isDuplicate": false }`                                                                |
